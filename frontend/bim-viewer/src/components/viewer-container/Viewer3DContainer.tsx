...
export default class Viewer3DContainer extends Vue {
  ...
  // URL du modèle glTF à charger automatiquement (voir ProjectPanel.modelUrl) :
  // si renseignée, on charge un projet synthétique au lieu des projets d'exemple.
  @Prop({ default: "" }) modelUrl!: string;
  ...
  mounted() {
    ...
    if (viewer.renderer) {
      ...
      if (this.modelUrl) {
        // Mode Typhon : projet synthétique porté par l'URL `?model=` — pas de
        // dépendance à config/projects.json ni au service backend du viewer.
        const proj: Project = {
          id: this.projectId,
          name: "Bâtiment — Typhoon",
          models: [{
            name: "Bâtiment",
            src: this.modelUrl,
            position: [0, 0, 0],
  ...
  /**
   * Mode Typhon (`?model=` dans l'URL) : bouton flottant « Projets » qui
   * ramene a la liste des projets d'exemple du viewer. L'URL du jumeau est
   * retenue dans sessionStorage pour permettre de revenir au batiment depuis
   * la liste (bouton « Retour au jumeau » cote Projects).
   */
  goToProjects() {
    // En mode Typhon (`?model=`), on retient l'URL du jumeau pour offrir un
    // « Retour au jumeau » depuis la liste. Un projet d'exemple n'a rien a
    // retenir : le bouton ramene simplement a la liste.
    if (this.modelUrl) {
      try {
        sessionStorage.setItem("typhoon_remote_model", this.modelUrl);
      } catch (e) {
        /* sessionStorage indisponible : on navigue quand meme */
      }
    }
