...
export default class Projects extends Vue {
  ...
  /**
   * Mode Typhon : Viewer3DContainer retient l'URL du jumeau dans
   * sessionStorage avant de venir sur cette liste. Si elle existe, on
   * affiche un bouton « Retour au jumeau » qui recharge /projects/remote
   * avec cette URL (retour au batiment sans relancer le diagnostic).
   */
  get remoteModelUrl(): string {
    try {
      return sessionStorage.getItem("typhoon_remote_model") || "";
    } catch (e) {
      return "";
    }
  }
  ...
  goBackToRemote() {
    const url = this.remoteModelUrl;
    try {
      sessionStorage.removeItem("typhoon_remote_model");
    } catch (e) {
      /* ignore */
    }
    if (url) {
      this.$router.push(`/projects/remote?model=${encodeURIComponent(url)}`);
    }
  }
  ...
  protected render(): VNode {
    ...
    return (
      <div ref="projects" class={styles.projects}>
        {this.remoteModelUrl && (
          <button class={styles.backToJumeau} onClick={() => this.goBackToRemote()} title="Revenir au jumeau numérique du bâtiment Typhoon">
            <i class="el-icon-back" />
            <span>Retour au jumeau</span>
          </button>
        )}
