...
/**
 * Ancres les URLs relatives (ex. "projects/technical_school/model.gltf" ou
 * une miniature "projects/.../thumbnail.png") sur la BASE du viewer
 * (/bim-viewer/). Sans cela, un chemin relatif est resolu contre la ROUTE
 * history courante du router Vue (/bim-viewer/projects/:id) et devient
 * "/bim-viewer/projects/projects/..." -> 404 (bug vu sur les projets
 * d'exemple). Les URLs absolues (http://, //, /, data:, blob:) sont
 * laissees telles quelles — c'est le cas du jumeau Typhoon (?model=...).
 */
