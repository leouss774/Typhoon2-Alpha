...
import { Model, ProjectManager, resolveAssetUrl } from "./ProjectManager";
...
export default class Viewer3D {
  ...
  async loadModel(options: Model, onProgress?: (event: ProgressEvent) => void, onError?: (event: ErrorEvent) => void): Promise<THREE.Object3D> {
    // Typhon : le src peut être une URL avec query (ex. .../gltf?q=...) —
    // l'extension se lit sur le chemin, pas sur l'URL complète (une adresse
    // finissant par "stl"/"ifc"/"obj"... routerait vers le mauvais loader).
    // ATTENTION : les loaders reçoivent options.src ENTIER (query comprise),
    // sinon le ?q= du modèle serait perdu (fetch 422 côté Typhoon).
    const src = resolveAssetUrl(options.src);
    const pathUrl = src.split(/[?#]/)[0].toLowerCase();
    if (pathUrl.endsWith("fbx")) {
      return this.loadFbx(src, options, onProgress, onError);
    } else if (pathUrl.endsWith("obj")) {
      return this.loadObj(src, options, onProgress, onError);
    } else if (pathUrl.endsWith("stl")) {
      return this.loadStl(src, options, onProgress, onError);
    } else if (pathUrl.endsWith("ifc")) {
      return this.loadIfc(src, options, onProgress, onError);
    } else if (pathUrl.endsWith("shp")) {
      return this.loadShp(src, options, onProgress, onError);
    } else if (pathUrl.endsWith("dae")) {
      return this.loadDae(src, options, onProgress, onError);
    } else {
      return this.loadGltf(src, options, onProgress, onError);
    }
  }
