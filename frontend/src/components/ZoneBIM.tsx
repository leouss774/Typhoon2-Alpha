...
export function ZoneBIM({
  ...
  /* Envoie le rapport au viewer dès que l'iframe est prête (et à chaque
     rechargement) : les simulations dérivent leurs niveaux de `aleas` et le
     coloriage « risques par partie » utilise `zones` (7 zones du moteur de
     risque). Les zones arrivent après le rapport (fetch dédié) : on renvoie
     le payload quand elles sont prêtes. */
  useEffect(() => {
  ...
  }, [loaded, report, attempt, recommendationZones]);
