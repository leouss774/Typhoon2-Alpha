...
Entrypoint FastAPI — expose le StateGraph LangGraph comme un service de
 diagnostic (cf. README racine, section "Backend — communication inter-agents").

 Lancement (port 8765 — convention repo, cf. README "port 8765 obligatoire" ; les fronts statiques l'appellent en dur) :
     cd backend
     uvicorn app.main:app --reload --port 8765

CORS ouvert (`allow_origins=["*"]`) : le front de test (`frontend/jumeau_numerique/index.html`) est ouvert directement en
`file://` depuis le navigateur (pas de serveur web devant), dont l'origine est "null" - un allow_origins restrictif casserait cet usage. A resserrer si un vrai front est deploye derriere un domaine.
"""
...
from app.api.routes import usine as usine_router
...
app.include_router(usine_router.router, tags=["usine"])
@app.on_event("startup")
async def on_startup() -> None:
    # Index de l'agent recommandations (~19 Mo, ~900 fiches) : charge une seule fois ici plutot qu'a chaque requete /diagnostic, cf. app/recommandations/service.py. Si l'index manque, on demarre quand meme (les recommandations resteront vides) — ne pas bloquer le service.
    try:
        load_index()
